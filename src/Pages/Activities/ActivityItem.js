...
function ActivityItem(props) {
    return (
        <div className="ActivityItem flex flex-row justify-between items-center bg-[#E7E5E5] text-sm md:text-base lg:text-lg font-bold py-3 px-4 mt-3 rounded-sm border-b-2 border-black w-[714px] md:w-full">
            <div className="min-w-[25%] truncate overflow-clip">{props.studentName}</div>
            <div className="min-w-[12.5%]">{props.studentId}</div>
            <div className="min-w-[12.5%]">{props.printerId}</div>
            <div className="min-w-[12.5%]">{props.location}</div>
            <div className="min-w-[12.5%]">{props.date}</div>
            <div className="min-w-[25%] flex gap-2">
